...
def send_email(recipient, body):
    ...
    gmail_user = 'grassroots.science.summer2021@gmail.com'
    gmail_pwd = 'C!v!lS0c!3ty'
    FROM = 'Guy Schaffer'
    TO = recipient if type(recipient) is list else [recipient]
    SUBJECT = 'New announcement from Grassroots Science'
...
async def on_message(message):
    if message.content == "!list":
        ...
        if str(message.author) != 'announcementBot#8020':
            for email in emails:
                send_email(email, message.content)


client.run('ODQ3NTA2NjMyNzQxMzU1NTIw.YK_EDw.7xE_O1GLGA46dsPzP_XNocM0n18')
